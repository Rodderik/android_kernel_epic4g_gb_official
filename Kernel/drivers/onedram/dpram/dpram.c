...
static inline void _memcpy(void *p_dest, const void *p_src, int size)
{
	...
	spin_lock_irqsave(&mem_cpy_lock, flags);
    
	if (!(*onedram_sem)) {
		printk(KERN_ERR "[OneDRAM] memory access without semaphore!: %d\n", *onedram_sem);
		spin_unlock_irqrestore(&mem_cpy_lock, flags);
		return;
	}
	if (size <= 0) {
		spin_unlock_irqrestore(&mem_cpy_lock, flags);
...
static inline int _memcmp(u8 *dest, u8 *src, int size)
{
	...
	spin_lock_irqsave(&mem_cmp_lock, flags);
	if (!(*onedram_sem)) {
		printk(KERN_ERR "[OneDRAM] memory access without semaphore!: %d\n", *onedram_sem);
		spin_unlock_irqrestore(&mem_cmp_lock,flags);
		return 1;
	}
...
//	dpram_clear();
	...
	retval= request_threaded_irq(dpram_irq,NULL, dpram_irq_handler, IRQF_TRIGGER_LOW | IRQF_ONESHOT,  "dpram_irq",NULL);
