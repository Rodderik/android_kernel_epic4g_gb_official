...
void WiMAX_I2C_Init(void)
{
	...
	gpio_set_value(EEPROM_SDA, GPIO_LEVEL_HIGH);
        s5p_gpio_set_drvstr(EEPROM_SCL,S3C_GPIO_DRVSTR_4X); // added for eeprom issue
	udelay(2);

	// start bit
	s3c_gpio_cfgpin(EEPROM_SDA, S3C_GPIO_SFN(1));
	gpio_set_value(EEPROM_SDA, GPIO_LEVEL_LOW);
        s5p_gpio_set_drvstr(EEPROM_SDA,S3C_GPIO_DRVSTR_4X); // added for eeprom  issue
	udelay(2);
...
void WiMAX_I2C_Deinit(void)
{
	// stop
	gpio_set_value(EEPROM_SCL, GPIO_LEVEL_LOW);
        s5p_gpio_set_drvstr(EEPROM_SCL,S3C_GPIO_DRVSTR_1X); // added for eeprom issue
	udelay(1);
	s3c_gpio_cfgpin(EEPROM_SDA, S3C_GPIO_SFN(1));
	gpio_set_value(EEPROM_SDA, GPIO_LEVEL_LOW);
	udelay(1);
	...
	udelay(1);
	gpio_set_value(EEPROM_SDA, GPIO_LEVEL_HIGH);
        s5p_gpio_set_drvstr(EEPROM_SDA,S3C_GPIO_DRVSTR_1X); //added for eeprom issue
...
void WiMAX_EEPROM_Fix(void)
{
        ...
        for (i = 0; i < 16; i++)
        {
                ...
                if (!(buf[i] == boot_image[i]))
                {
                        is_eeprom_error = 1;
			DumpDebug("Previous image not matching  ! Writing new Image -----> %d",is_eeprom_error);
                        break;
                }
        }
#else
...
//
#if defined (CONFIG_MACH_ARIES) || defined (CONFIG_MACH_VICTORY)	//cky 20100225
